...
The extension is a single Node bundle (`dist/extension.js`) built by esbuild from `src/extension.ts`. It registers **one** webview view (`acurastStudio` of type webview, named "Home") in the Acurast Studio activity bar container. The webview is a **Svelte 5** SPA (`dist/studio/webview.js`) with an internal router (`home | wallets | settings | deploy | history | processors | tunnel`) — the root `App.svelte` (kept at the webview root) holds all reactive state and dispatches to seven route components. The webview is organized **one folder per route** (`src/studio/webview/<route>/<Route>.svelte`); components shared across routes live in `src/studio/webview/shared/` and pure-TS utilities in `src/studio/webview/lib/`. Global styles live in `media/studio/global.css`; `studioPanel.ts` reads `media/studio/webview.html` and substitutes `{{CSP}}`, `{{STYLE_URI}}`, `{{SCRIPT_URI}}`, `{{NONCE}}` sentinels at runtime. The webview talks to the host via `postMessage`; the host delegates to existing command implementations (`vscode.commands.executeCommand('acurast.wallet.create', id)` etc.) rather than re-implementing flows. Services (wallet storage, SDK client, project config) live outside the panel so commands triggered from the palette or status bar share the same backing logic.
...
- **`src/sdk/distroFetch.ts` + `src/sdk/distros.ts`** — The proot-distro image catalog behind the Shell runtime's image dropdown in Settings (a Shell deploy needs a rootfs `.tar.xz` URL *and* its SHA256; users previously hand-copied both). `distros.ts` is **generated** by `npm run build:distros` (see `scripts/build-distros.mjs`, which transpiles `distroFetch.ts` with esbuild and imports it, so the bundled catalog and the runtime refresh share one parser). `distroFetch.ts` re-reads it live when the user hits Refresh. See the sharp edge below before touching either.
...
- **`src/studio/studioPanel.ts`** — Host class (~900 lines). Holds a `_route` field and posts `{type: 'route'}` to the webview on `navigate(route)`. Polls balance every 30s while on the `wallets` route. Sets context key `acurast.studio.route` so `view/title` menus can hide/show the Home button. The `html()` method reads `media/studio/webview.html` and replaces the four `{{...}}` sentinels — no inline template. Handles history messages: `history.load` (paginated local records), `history.fetchOnline` (partial-key chain query for the active wallet's jobs), `history.delete` (consolidated delete: removes the local record and signs an on-chain deregister when the job is still registered), `history.bulkDelete` (multi-select: one confirm + one password, then one `utility.forceBatch` of deregisters per network; per-item outcomes via `deregister.state`), `history.removePathInfo`, `history.openFolder`.
...
- **`src/studio/webview/App.svelte`** — Svelte 5 root component. Holds all global state with `$state` runes (`route`, `wallets`, `balance`, `config`, `deploy`, `ctx`, `historyState`). Registers the `window.addEventListener('message')` handler (typed as `OutMsg`) in a `$effect` with cleanup. Routes to the seven page components via `{#if}` blocks. Stays at the webview root; every other route lives in its own folder.

- **Route components — one folder per route** (under `src/studio/webview/`): `home/Home.svelte`, `wallets/Wallets.svelte`, `settings/Settings.svelte`, `deploy/Deploy.svelte`, `history/History.svelte`, `processors/Processors.svelte`, `tunnel/Tunnel.svelte`. Each receives typed props; uses `$derived`/`$derived.by` for computed values. All DOM events use `onclick={}` (Svelte 5 syntax — not `on:click`). Components used by more than one route live in **`src/studio/webview/shared/`** (`DiagnosisPanel`, `DiagnoseButton`, `NetworkMismatchBanner`, `Spinner`, `FiatNote`); pure-TS utilities in **`src/studio/webview/lib/`**.

- **`src/studio/webview/history/History.svelte`** — Two-section layout: **Local** (paginated via `history.load` with offset, accumulated client-side) and **On-chain** (accordion, fetched on demand via `history.fetchOnline`, client-side paginated). On-chain cards decode `JobRegistration` fields (schedule, slots, reward, modules, script URL) and show a derived status badge (active/scheduled/expired) from `startTime`/`endTime`.
...
- **The proot-distro catalog has three upstream quirks — don't "simplify" them away.** (1) Tarballs are pinned **per distro, not per release** (at v4.38.0, Ubuntu points at a v4.37.0 build while Void still points at v4.29.0), so the catalog cannot be derived by listing one release's assets — the `distro-plugins/*.sh` files at a tag *are* the manifest. (2) Only the **v4** line still publishes a *catalog*; the v5 rewrite dropped the bundled distro list (`proot-distro install` now takes a Docker ref, a URL, or a local archive). The maintainer does mirror v5 rootfs tarballs at `easycli.sh/proot-distro/v5.0.0/`, but publishes **no SHA256** for them (v5 never verifies a hash — Acurast does), and two of those filenames aren't build-pinned (`ubuntu_questing_…`, `debian_trixie_…`), so upstream can overwrite them in place and silently invalidate a pinned hash. That makes the v4 plugin files the only source pinning URL **and** SHA256 together, so we resolve the newest `v4.x` tag, not the newest tag — don't "upgrade" this to v5. (3) Since **v4.37.0** the tarballs are no longer on GitHub — the maintainer migrated downloads to their own host (`easycli.sh`, commit `7bfb52c7`), and no GitHub release has been cut since v4.34.2. We therefore offer **two groups** in the dropdown — the current images and the newest still-on-GitHub ones (frozen at v4.36.0) — and label each with its download host, so the user picks their own availability tradeoff. Integrity is unaffected either way: the SHA256 comes from GitHub (the plugin file) and the processor verifies it on download, so the host only affects availability, never trust. Only `aarch64` is offered — Acurast processors are Android ARM64 devices.
...
- **New webview route**: (1) Add the route name to the `Route` type in `src/studio/types.ts`. (2) Create `src/studio/webview/<route>/<Route>.svelte` with typed `$props()` (one folder per route; cross-route components go in `shared/`, pure TS in `lib/`). (3) Import and add a `{:else if route === 'myroute'}` branch in `App.svelte`. (4) Update the `routeTitles` map in `App.svelte`. (5) Set `acurast.studio.route` context key in `studioPanel.ts` for any `view/title` buttons that should appear only on that route.
